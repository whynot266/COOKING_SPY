...
public class TodayDietController {
    ...
    @RequestMapping(value = "/addFoodToDiet", method = RequestMethod.POST)
    public @ResponseBody int addFood(@RequestParam(name = "foodId") long foodId, HttpSession session) {
        ...
        if (todayDietService.checkFoodExistence(foodId, user)) {
            todayDietService.increaseCoefficientByFoodIdAndUserBy1(foodId, user);
            return (int)session.getAttribute("dietCount");
        }else{
            session.setAttribute("dietCount", (int)session.getAttribute("dietCount")+1);
    ...
    public String deleteFood(@RequestParam(value = "id") long id, HttpSession session) {
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String username = principal.toString();
        if (principal instanceof UserDetails) {
            username = ((UserDetails) principal).getUsername();
